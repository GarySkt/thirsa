...
<div id="contenedor">
  ...
  <div id="banner">
	...
	$varOrden=$_SESSION['IdOrden'];	
	$varCliente = $_SESSION['cliente']['nom']. ' '. $_SESSION['cliente']['ape'];
	...
	else if (!empty($_SESSION['cliente']) && empty($_SESSION['IdOrden']))
	{
		?>
        <article>
        <p>Ud no ha hecho pedidos aun</p> 
        </article>
        <?php
	}
	else
	{
        ...
        <p>Usuario: <?php echo $varCliente; ?> </p>
        <p>Fecha: <?php echo $Fecha; ?></p>
        </div>
        
        <article>
          <table border="1" bordercolor="#906" cellspacing="0">
            ...
            {		
                ...
                ?>
                <tr>    
                    <td><?php echo $num['nombreproduc'];?></td>
                    <td><?php echo $varCantidad; ?></td>
                    <td><?php echo $varPrecio; ?></td>
                    <td><?php echo $PrecioTotal;?></td>
                </tr>
                <?php
               $bruto=$bruto + $PrecioTotal;
            }
            echo "</table>";
            $igv=$bruto * 0.19;
            $total = $bruto + $igv;
            ...
            <div class="monto">
            <ul>
                <li><?php echo 'S/. '. $bruto;?></li>
                <li><?php echo 'S/. '. $igv;?></li>
                <li><?php echo 'S/. '. $total;?></li>
            </ul>
            </div>
            
              
            <?php
            $_SESSION['montos'] = array('bruto' => $bruto, 'igv'=> $igv, 'total' =>$total);
            ?>	
            <form method="post" action="enviar_pedido.php" name="form_pedido">
            Observacion<br>
            <textarea name="observacion" cols="50" rows="5"></textarea> <br>          
