...
<div id="contenedor">
  ...
  <div id="banner">
	...
	<?php
		$id_cat=$_GET['cat'];
		$query_cat = mysql_query("SELECT * FROM categorias WHERE codcategoria='$id_cat'");
		$num_filas = mysql_fetch_assoc($query_cat);
	?>
    <div class="titulo">        
		<?php
			if($num_filas)
			{
				echo $num_filas['nom_categoria'];
			}		     
		?>          
    </div>
    ...
    <table width="500" align="center">
    	<?php		
		$query_prod = mysql_query("SELECT * FROM productos WHERE codcategoria='$id_cat'");
		$contador=0;
		echo "<tr>";
			while($campo = MySQL_fetch_array($query_prod))
			{
				if($contador==2)
				{
					echo "</tr>";
